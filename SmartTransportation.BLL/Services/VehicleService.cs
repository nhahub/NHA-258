    ...
    public class VehicleService : IVehicleService
    {
        ...
        public async Task<VehicleDTO> CreateVehicleAsync(int driverId, CreateVehicleDTO dto)
        {
            ...
            var entity = new Vehicle
            {
                ...
                Color = dto.Color,
                // ✅ Convert int? from DTO to int? for entity (safe assignment)
                SeatsCount = (int)dto.SeatsCount,
                VehicleLicenseNumber = dto.VehicleLicenseNumber,
                // ✅ Convert DateTime? from DTO to DateOnly? for entity
                VehicleLicenseExpiry = dto.VehicleLicenseExpiry.HasValue
                    ? DateOnly.FromDateTime(dto.VehicleLicenseExpiry.Value)
                    : null,
                IsVerified = false,
        ...
        public async Task<VehicleDTO> UpdateVehicleAsync(int vehicleId, UpdateVehicleDTO dto)
        {
            ...
            // ✅ Fixed: Check HasValue and assign int?
            if (dto.SeatsCount.HasValue && dto.SeatsCount.Value > 0)
                entity.SeatsCount = (int)dto.SeatsCount;

            if (!string.IsNullOrWhiteSpace(dto.VehicleLicenseNumber))
                entity.VehicleLicenseNumber = dto.VehicleLicenseNumber;

            // ✅ Convert DateTime? from DTO to DateOnly? for entity
            if (dto.VehicleLicenseExpiry.HasValue)
                entity.VehicleLicenseExpiry = DateOnly.FromDateTime(dto.VehicleLicenseExpiry.Value);
